...
LOG_DIR="/home/mika/log-dir" # Path to log folder
if [ $? -ne 0 ]; then
...
if [ "$available_space" -le 10 ]; then
	echo -e "${RED}Your available space in C: is less than 10%${RESET}"
	echo "Warning: low amount of space in C: $available_space% Date: $(date -I)" >> "$LOG_DIR/disk-space.log"
fi
